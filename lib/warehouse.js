...
Warehouse.prototype.add_to_stack = function(stack_name, path, fn){
  ...
  if(path){
    ...
    var path_match = path.match(/^(\w+)?(:\/\/)?(.*?)$/);
    var protocol = path_match[1];
    var packetpath = path_match[3];

    function pathmatch(packet){

      if(protocol && protocol!=packet.protocol()){
        return false;
      }
...
Warehouse.prototype.build_supplychain = function(){
  var self = this;

  function supplychain(packet, next){
  ...
  supplychain._issupplychain = true;
  supplychain.on = _.bind(this.on, this);
  supplychain.off = _.bind(this.off, this);
  supplychain.trigger = _.bind(this.trigger, this);
  supplychain.radio = radio;

  /*
    The radio broadcasts events for this warehouse
    by connecting to a telegraph

   */

  self._radio = radio;
  self._supplychain = supplychain;

}

Warehouse.prototype.radio = function(){
  return this._radio;
}
...
Warehouse.prototype.register_supplychain = function(fn){
  ...
  if(fn._issupplychain){
    fn.on('message', function(message){
