 ...
 */
function builder(options, ready){
  options || (options = {})
...
function factory(options){
  ...
  function new_supplier(req, ready){
    ...
    supplier_options.stamp = req.path();
    supplier_options.path = req.lastPath();
    supplier_options.route = req.originalPath();
    supplier_options.network = req.network;
    builder(supplier_options, ready);
  }
      ...
      if(req.param('selector')){
        resolver({
          ...
          skeleton:[{
            ...
            meta:{
              quarryid:req.extract_supplier_quarryid(supplier.get('path'))
            }
          }]
