...
function factory(options, ready){
  ...
  var supplier = {};
  _.extend(supplier, Backbone.Events);
  _.extend(supplier, Proto);
  _.extend(supplier, api);

  var init_fn = supplier.init;
  var arity = init_fn.length;
  var supplier_options = _.clone(options);

  // is the supplier init function gonna take some time?
  if(arity>=1){
    supplier.init(supplier_options, function(){
      ready(null, supplier);
    })
  }
  // no we are good right away
  else{
    supplier.init(supplier_options);
    ready(null, supplier);
  }
