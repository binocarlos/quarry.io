...
Warehouse.get_stack_entry = function(route, fn, options){
  var self = this;
  // default route to '/'
...
Warehouse.usebefore = function(route, fn, options){
  var entry = this.get_stack_entry(route, fn, options);
  this.stack.unshift(entry);
  return this;
}

Warehouse.use = function(route, fn, options){
  if(!route && !fn){
...
_.each(['head', 'get', 'post', 'put', 'delete'], function(verb){
  Warehouse[verb] = function(route, fn, options){
    var self = this;
    var entry = this.get_stack_entry(route, fn, options);
...
Warehouse.get_handler = function(){
...
Warehouse.handle = function(req, res, out) {
  ...
  function next(err) {
    ...
    if(layer && layer.debug){
      console.log('-------------------------------------------');
      console.log('-------------------------------------------');
      console.log('RUNNING LAYER: ' + layer.route);
...
Warehouse.assign_network_client = function(network){

}

Warehouse.automount = function(middleware){
  middleware && this.use(middleware);
}

Warehouse.setup = function(){
  
}

Warehouse.boot = function(){
 ...
 */
Warehouse.inject_deployment_to_requests = function(deployment){

  var switchboard = deployment.pubsub('/reception/switchboard/'); 
  ...
  this.usebefore(function(req, res, next){
    ...
    req.bootstrap({
      ...
      direct:function(location){
        var supplychain = deployment.rpc(location);
      ...
      redirect:function(location){
        ...
        */
        if(!req.bayid()){
          ...
          var supplychain = deployment.rpc('/');
          req.path(location);
          supplychain(req, res, next);
        }
        else{
