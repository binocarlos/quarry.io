...
// get set the supplychain for this warehouse
Warehouse.supplychain = function(chain){
...
Warehouse.handle = function(req, res, out) {
  ...
  req.route = function(){
    self.emit('router', 'route', req);
  }

  // add another query to the holding bay
  req.branch = function(){
    self.emit('router', 'branch', req);
  }

  // send a message to the switchbaord
  req.broadcast = function(){
    self.emit('router', 'broadcast', req);
  }
