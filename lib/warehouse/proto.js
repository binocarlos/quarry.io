...
_.each(['head', 'get', 'post', 'put', 'delete'], function(method){
  Warehouse.prototype[method] = function(route, fn){
    ...
    console.log('-------------------------------------------');
    console.log('method mount: ' + method + ':' + route);
...
Warehouse.prototype.handle = function(req, res, out) {
  ...
  function next(err) {
    ...
    //try {
      ...
      console.log(path + ' running ' + (!_.isEmpty(layer.handle.name) ? layer.handle.name : 'anonymous'));
...
/*
  ...
  provide a container that is 'connected' to this
  warehouse via its handle function
  
*/
Warehouse.prototype.connect = function(fn){
