...
/*
  ...
  A warehouse that looks after a cache of suppliers each created based on the path
 ...
 */
function factory(get_supplier_function){

  var provider = Warehouse();

  // the cache of warehouses we have already created
  var cache = {};

  function get_supplier(path, callback){
    console.log('-------------------------------------------');
    console.log('get supplier');
    if(cache[path]){
      return cache[path];
    }
    var supplier = get_supplier_function(path);
    if(supplier){
      cache[path] = supplier;  
    }
    return supplier;
