...
var Request = module.exports = BackboneDeep.extend({
  initialize:function(forceid){
    ...
    if(_.isString(this.param('selectors'))){
      this.selectors(selectorParser(this.param('selectors')));
      this.unset('params.selectors');
    }

    if(_.isString(this.param('selector'))){
      this.selector(selectorParser(this.param('selector')));
      this.unset('params.selector');
    }    
  },
  ...
  transactionindex:utils.object_property_accessor('headers', 'X-QUARRY-TRANSACTION-INDEX'),
  transactionroute:utils.object_property_accessor('headers', 'X-QUARRY-TRANSACTION-ROUTE'),
  ...
  copyinto:function(req){
    ...
    req.transactionroute(this.transactionroute());
    req.transactionindex(this.transactionindex());
  ...
  stampResponse:function(res){
    ...
    res.transactionroute(this.transactionroute());
    res.transactionindex(this.transactionindex());
