...
function queryFactory(supply_chain, selector, context){
  ...
  function trigger(){
    ...
    // here is where we map the raw results onto containers
    // this is also where we should add to the route
    function results_callback(error, res){
      result_container = factory(res.answer.results, supply_chain);
