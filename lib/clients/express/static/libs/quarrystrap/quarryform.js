!function ($, $quarry) {
    ...
    container.bind_dom(elem.find('.selector-classnames'), {
      ...
      datasource:function(val){
        if(val){
          ...
          container.classnames(new_classnames);
        }

        
        return _.keys(container.classnames() || {}).join(' ');
      }
    })
  ...
  function text(field, container){
    ...
    var wrapper = field_wrapper(dom, {
      id:field_id,
      title:field.title
    })

    container.bind_dom(dom, {
      field:field,
      auto_event:'change'
    }).on('error', function(){
  ...
  $.extend({
    quarryform:function(options){
      ...
      form.element.validate = _.bind(form.validate, form);
