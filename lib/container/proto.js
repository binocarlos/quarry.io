...
Container.prototype.portal = function(){
  ...
  if(skeleton.length>0 && this.supplychain){
    portal.attach(skeleton[0], this.supplychain.switchboard);
  }

  return portal;
}

Container.prototype.route = function(type){
  type || (type = 'rpc');
  var routes = Router(this.skeleton());
  return routes[type];
}
  ...
  function append_single_container(childcontainer){
    ...
    */
    ...
    self.each(function(parentcontainer){
      ...
      var route = parentcontainer.route();
