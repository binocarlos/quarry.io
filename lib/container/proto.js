...
Container.prototype.append = function(childcontainer){

  var self = this;

  /*
  
    if we actually have an array of containers then merge all their models
    into a new container as though we had passed one with all the models in it already
    
  */
  if(_.isArray(childcontainer)){
    var mergedmodels = [];
    _.each(childcontainer, function(actualcontainer){
      mergedmodels = mergedmodels.concat(actualcontainer.models);
    })
    childcontainer = childcontainer[0].spawn(mergedmodels);
  }
  ...
  */
  ...
  this.each(function(container){
    ...
    var child_models = _.map(childcontainer.models, childmodelmap);
    model.children || (model.children = []);
    model.children = model.children.concat(child_models);
