...
Container.portal = function(fn){
  ...
  console.log('-------------------------------------------');
  console.log('portal');
  eyes.inspect(self.switchboards);
...
Container.append = function(append_containers){
  ...
  this.models.each(function(parent_model){
    ...
    rollbacks[parent_model.get('meta.quarryid')] = function(){
      parent_model.remove(appended_models);
    }
  })

  var model_ids = this.models.map(function(parent_model){
    return parent_model.get('meta.quarryid');
  })
  ...
  query.body(_.map(children_models, function(children_model){
