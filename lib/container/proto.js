...
Container.prototype.addbranch = function(config, target){
  ...
  var link = target.router().link();

  var branches = this.meta('branches');

  branches[link.quarryid] = {
    link:link,
    config:config
  }

  this.meta('branches', branches);
...
/*
  ...
  this is exposed to bootstrap container warehouses in the browser
  
*/

Container.prototype.bootstrapbrowser = function(containerconfig){
  /*
  ...
  */
  ...
  socket.on('connect', function(){
    _.each(readybreks, function(readybrek){
      readybrek();
    })
    readybreks = [];
    isreadybrek = true;
  ...
  */
  ...
  function getcontainer(appendpath){
    appendpath = arguments.length>=0 ? (appendpath.indexOf('/')==0 ? appendpath : '/' + appendpath) : null;
    ...
    var retcontainer = Container.new(containerdata);
    /*
  ...
  function $quarry(stackpath, fn){
    if(isreadybrek){
        fn && fn(container);
        return this;
      }
      readybreks.push(function(){
        fn && fn(container);
      })
      return this;
  }
  ...
  $quarry.ready = $quarry;
  $quarry.new = Container.new;

  $quarry.series = async.series;
  $quarry.parallel = async.parallel;
  $quarry.waterfall = async.waterfall;
  $quarry.forEach = async.forEach;
  $quarry.forEachSeries = async.forEachSeries;
