...
Container.prototype.portalkey = function(routingkey){
  ...
  var route = this.route().replace(/^\//, '').replace(/\//g, '.');

  var parts = ['container:/' + route];
...
Container.prototype.hasroute = function(method){
...
Container.prototype.route = function(method, value){
  ...
  var route = routes[method] || routes.normal || '/';

  /*
  
    we return the route with the quarryid appended (unless it's a supply chain
    in which case the route has been set by the supplychain itself as per
    it's connection on the network)
    
   */
  return this.tagname()=='supplychain' ? route : route + '/' + this.quarryid();
}

/*

  return a container that is a symlink to the original container
  
*/
Container.prototype.link = function(){
  var linkdata = _.map(this.containers(), function(container){
    return {
      meta:{
        tagname:'symlink',
  ...
  _.each(this.models, function(model){
    ...
    */
    var children = _.map(model.getChildren(), function(childmodel){
      var route = deepdot(childmodel, 'meta.routes.normal');
      if(!route){
        deepdot(childmodel, 'meta.routes.normal', self.route());
      }
    })
    all_models = all_models.concat();
  })
...
*/
function wrapper(key, subkey){

  var observe = true;

  var fullkey = arguments.length>1 ? key + '.' + subkey : key;
  ...
  return function(){
    ...
    */
    if(arguments.length<=0){
      return deepdot(this.models[0], fullkey);
    }
    /*
    ...
    */
    else if(arguments.length==1 && _.isString(arguments[0]) && !subkey){
    ...
    */
      ...
      var val = arguments[0];
      _.each(this.models, function(model){
        deepdot(model, fullkey, val);
        if(observe){
          deepdot(model, 'changed.' + fullkey, val);
        }
      })
      return self;
    }
    ...
    */
      ...
      var name = [fullkey, arguments[0]].join('.');
