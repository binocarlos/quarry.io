...
Container.update_children = function(children, remove){
  var self = this;
  if(_.isArray(children)){
    _.each(children, function(child){
      self.update_children(child, remove);
    })
    return this;
  }
  else{
    this.models.each(function(model){
      if(remove){
        model.remove(children.basemodels());
      }
      else{
       model.append(children.basemodels());
      }
    })
    return this;
  }
...
Container.append = function(children, callback){
  ...
  // we are running in network mode
    ...
    var routes = this.getRoutes(true);

    var transaction = new Transaction({
      container:this,
      query:queries.append(routes, raw),
      routes:routes,
      handle:_.bind(self.handle, self),
      rollback:function(container){
        console.log('-------------------------------------------');
        console.log('-------------------------------------------');
        console.log('error from transaction: rollback');
        console.log(container.quarryid());
        //self.update_children(children, true);
      },
      commit:function(container, update){
        console.log('-------------------------------------------');
        console.log('-------------------------------------------');
    ...
    var req = queries.append(routes, raw);
