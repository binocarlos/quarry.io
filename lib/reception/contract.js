...
Contract.finish = function(){
  var res = queries.response(this.finished_callback);

  res.sendCombo(this.results, this.errors);
}
...
Contract.prototype.finish_request = function(req, res){
  var self = this;
  if(res.hasError()){
    self.errors.push(res.body());
  }
  else{
    self.results.push(res.body());
  }
