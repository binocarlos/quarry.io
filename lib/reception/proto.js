...
Reception.transaction = function(mainreq, mainres, next){
  ...
  var route_method = mainreq.transactionroute() || 'post';
  ...
  */
   ...
   */
  var skeleton_array = _.map(mainreq.jsonheader('X-QUARRY-SKELETON') || [], function(skeleton){

    // get the route for 'into' the container because we are
    return router.skeleton(skeleton, route_method);
  })
   ...
   */
  var branches = _.map(skeleton_array, function(skeleton){

    var branch_req = queries.fromJSON(JSON.parse(JSON.stringify(body)));
    branch_req.refreshid();

    /*

      We want the post route because we going selecting 'into' the container
    ...
    /*
      ...
      this means the originalPath will be what is the original branch
      
    */
    branch_req.resetroute(':reception', skeleton.route);
    branch_req.skeleton([skeleton]);

    eyes.inspect(branch_req.toJSON());
