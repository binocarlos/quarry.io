    ...
    mount:function(app, callback){
      ...
      */
      passport.serializeUser(function(user, done) {
        ...
        cache.set(user.id, JSON.stringify(user), function(error){
          if(!error){
            done(null, user.id);
          }
          else{
          ...
          function loaduser(id){
            db('user=' + id + ':tree').ship(function(user){
              ...
              /*
                ...
                this is what we emit to any mappers setup for the auth server
              ...
              */
              var rawuser = {
                ...
                attr:user.attr(),
                meta:user.meta(),
                children:user.find('profile').map(function(profile){
                  var name = profile.attr('provider');
                  return providers[name].map(profile);
                })
              }
          ...
          db('profile.' + name + '#' + rawprofile.id)
            .ship(function(profile){
              ...
              if(profile.count()<=0){
                ...
                */
                if(req.user){
                  ...
                  */
                  db('user=' + req.user.id).ship(function(user){
              ...
              */
                ...
                profile.attr(rawprofile).save().ship(function(){
