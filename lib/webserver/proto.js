...
function Webserver(options){
  ...
  if(!this.supplychain){
    throw new Error('Webserver needs a supply chain');
  }
...
Webserver.prototype.addwebsite = function(website_config, ready_callback){
  ...
  website_config.document_root = path.resolve(this.options.worker.stack.codefolder, website_config.document_root);
  ...
  website.initialize(this.supplychain, function(error){
    if(error){
      next_website(error);
      return;
    }
    ...
    _.each(website.hostnames(), function(hostname){
      ...
      self.router.use(express.vhost(hostname, website.app));
    })
...
Webserver.prototype.bind = function(ready_callback){
  ...
  this.load_websites(function(error){
    ...
    self.router.use(function(req, res, next){
      res.send('website not found for stack: ' + self.options.stack_id);
    })

    log.info('web server binding on port: ' + dye.red(self.options.endpoint.port));

    self.server.listen(self.options.endpoint.port, function(){
      ready_callback();
    })
