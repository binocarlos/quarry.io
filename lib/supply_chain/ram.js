...
function factory(options, ready_callback){
  ...
  */
   ...
   */
  actions.contract = function(req, callback){
    ...
    actions.select({
      ...
      actions.select({
        message:{
          ...
          previous:_.map(res.answer.results, function(result){
            return result._meta.quarryid;
          })  
        }
      }, function(error, res){
   ...
   */
  actions.select = function(req, callback){
    ...
    console.log('HERE');
    console.dir(search_within_containers.length);
    var all_results = [];
   ...
   */
  actions.append = function(req, callback){
