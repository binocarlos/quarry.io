...
Stack.prototype.get_layout = function(){
  ...
  */
  ...
  var webserver = Container.new('webserver', options.webserver.config);
  var reception = Container.new('reception', options.reception.config);
  var apiserver = Container.new('apiserver', options.apiserver.config);

  root.append([workers, webserver, reception, apiserver]);
...
Stack.prototype.upload = function(container, callback){
  ...
  container
    ...
    .ship(function(){
      ...
      self.webserver = layout.find('webserver');
      self.reception = layout.find('reception');
      self.apiserver = layout.find('apiserver');
  ...
  async.series([
    ...
    */
    function(next){
      ...
      self.boot_receptions(next);
      
    },

    /*
    
      then the api servers
      
    */
    function(next){

      self.boot_apiservers(next);
...
*/
Stack.prototype.boot_receptions = function(callback){
  var self = this;
  log.info('booting reception nodes');
  ...
  */
  async.forEach(this.workers.find('.reception').containers(), function(receptionworker, nextworker){
    ...
    var nodeconfig = _.extend({}, self.reception.attr(), {
      flavour:'reception',
      endpoints:self.allocation.get_reception_endpoints(receptionworker)
    })
