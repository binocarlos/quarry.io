...
*/
Stack.prototype.initialize = function(infrastructure, options){
  var self = this;
  options || (options = {});
    ...
    */
    function(next){
      async.forEachSeries(quarry, function(type, next){
        ...
        self.create_workers(type, allocation[type], next);
  ...
  */
  var worker = Container.new('worker')
    ...
    /*
      ...
      this is not a link at the moment
...
Stack.prototype.boot_worker = function(worker, callback){
  ...
  self.infrastructure.deploy_worker(worker, function(error){
    console.log('-------------------------------------------');
