...
Network.prototype.initialize = function(options, callback){
  ...
  /*
    ...
    the folders from which we get our stack code from
...
Network.prototype.slurp = function(path){
  ...
  fs.existsSync(path) && this.slurpedstacks.push(path);
...
Network.prototype.build_stack = function(folder){
...
*/
Network.prototype.get_layout = function(options){
  var self = this;
...
*/
Network.prototype.deployment = function(deployment){

  var self = this;

  if(!deployment){
    log.error('There is no deployment provided for the network');
    return;
  }
...
Network.prototype.boot = function(fn){
  ...
  async.series([
    ...
    */
    ...
    function(next){
      log.info('preparing the deployment');
      self.deployment.prepare(next);
    },
    ...
    */
    ...
    function(next){
      log.info('preparing the layout db');
      self.layout = self.get_layout(self.options);
      next();
    },
    ...
    */
    ...
    function(next){
      log.info('preparing the nameserver');
      self.nameserver = new NameServer();
      self.nameserver.reset(next);
    },
    ...
    */
    ...
    function(next){
      ...
      */
      self.system = new System(self.layout, self.deployment);

      //self.system.on('portal', _.bind(self.deployment.systemportal, self.deployment));

      self.system.bind(next);
      
    },

    /*
    
      upload the system database

      this creates us a QuarryDB supplier that will keep track of our network
    ...
    */
    ...
    function(next){
      ...
      self.system.upload(self.layout, next);
    ...
    /*
      ...
      this will add each stack to the system
      which will hook up the event listeners on the database

      the deployment will listen to these events and build out
    ...
    */
    function(next){
      ...
      */
      async.forEach(self.slurpedstacks, function(stackfolder, next_stack){

        log.info('uploading stack: ' + dye.green(stackfolder));
        ...
        stack.upload(self.system, next_stack);
    ...
    /*
      ...
      hook up the stack portal listeners to the deployment
    ...
    */
    ...
    function(next){
      ...
      /*
         ...
         load up the stacks and provide each to the deployment for hooking
         up the portals
        
      */
      var db = self.system.db;

      console.log('-------------------------------------------');
      console.log('loading instacnes');
