...
Network.prototype.slurp = function(path){
  ...
  fs.existsSync(path) && this.options.stackcontainer.localfolders.push(pathutils.normalize(path));
...
Network.prototype.boot = function(fn){
  ...
  async.series([
    ...
    */
    ...
    function(next){
      log.info('preparing the system in: ' + self.options.infrastructure.flavour + ' mode');
