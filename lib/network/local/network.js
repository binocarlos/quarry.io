...
var LocalNetwork = module.exports = Base.extend({
  ...
  */
  ...
  deploy_webserver:function(stack_id, loaded){
    ...
    fs.writeFileSync(this.get('folder')+'/build/' + stack_id + '/webserver.json', JSON.stringify(webserver_config, null, 4), 'utf8');
  ...
  post_booted:function(callback){
    ...
    */
    .concat(_.map(_.keys(this.stacks), function(stack_id){
      return (stack_id.replace(/\./g, '_') + '_webserver') + ' = node ' + booter + ' webserver --deployment=' + (self.get('folder')+'/build/' + stack_id + '/webserver.json');
    }))
    /*
