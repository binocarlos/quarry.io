...
*/
Stack.prototype.create_workers = function(type, instance, callback){
  var self = this;
  ...
  */
  ...
  self.workers.append(worker).ship(function(){
    ...
    self.bindworker(worker, callback);
  })
...
/*
  ...
  setup the portals onto a worker so that when instance allocations are added to it we react
  and tell the infrastructure to boot it
  
*/
Stack.prototype.bindworker = function(worker, callback){
  var self = this;

  log.info('setup worker portals: ' + worker.summary());
  ...
  */
  ...
  var test = Container.new('test');

  worker.append(test).ship(function(){
    console.log('-------------------------------------------');
    console.log('after');
    process.exit();
  })
