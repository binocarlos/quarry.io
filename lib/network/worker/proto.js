...
Worker.prototype.boot = function(callback){
  var self = this;
  this.system = Device('supplychain.client', self.options.system);
  this.system.connect(function(db){
    ...
    */
    db('worker=' + self.options.workerid).ship(function(worker){
      ...
      portal.sync(true, function(){
        console.log('-------------------------------------------');
        console.log('sync portal callback');

        console.log(JSON.stringify(worker.toJSON(), null, 4));
        
      })
/*
      var radio = portal.radio();

      radio.listen(function(){
        console.log('-------------------------------------------');
        console.log('-------------------------------------------');
        console.log('radio broadcast');
      })
*/
      callback();
    })
