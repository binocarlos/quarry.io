  ...
  this.system.connect(function(db){
    ...
    */
    db('worker=' + self.options.workerid).ship(function(worker){
      ...
      worker.portal()
        .appended('caption.apple', function(product){
          console.log('-------------------------------------------');
          console.log('-------------------------------------------');
          console.log('worker portal');
          eyes.inspect(product.toJSON());
        })

      var caption = Container.new('caption', {
        test:10
      }).addClass('apple')

      var test = Container.new('product', {
        test:10
      })

      test.append(caption)

      worker.append(test).ship()
