...
Section.prototype.build_default_worker = function(instance, callback){
  ...
  log.info(dye.magenta('create_worker: ' + this.flavour));
  ...
  */
  self.workerdb.append(worker).ship(function(){
    ...
    log.info(dye.magenta('worker added: ' + worker.summary()));
...
Section.prototype.boot_worker = function(worker, callback){
  ...
  log.info(dye.magenta('booting worker: ' + worker.summary()));
  ...
  this.emit('deployworker', {
    ...
    log.info(dye.magenta('worker booted: ' + worker.summary()));
...
Section.prototype.build_default_node = function(callback){
  ...
  */
  ...
  _.each(self.servicedb.attr('services') || [], function(serviceconfig){
    var service = Container.new('service', serviceconfig);
    self.map_service(service);
    self.defaultnode.append(service);
  })

  self.add_extra_services(self.defaultnode, function(){
    self.defaultnode.find('service').each(function(service){
      service.addClass(self.flavour);
    })
    ...
    */
    self.servicedb.append(self.defaultnode).ship(function(){
...
Section.prototype.map_service = function(service){
  var self = this;
  
  var codefolder = self.stackconfig.config.codefolder;
  
  _.each(service.attr(), function(val, prop){
    if(!_.isString(val)){
      return;
    }
    if(val.indexOf('__stack.codefolder')==0){
      val = val.substr('__stack.codefolder'.length);
      val = codefolder + val;
      service.attr(prop, val);
    }
  })
...
/*
  ...
  add any extra services here before everything boots
  
*/
Section.prototype.add_extra_services = function(node, callback){
  callback(null, node);
}
  ...
  async.forEach(workers.containers(), function(worker, nextworker){
    ...
    */
    ...
    async.forEach(_.keys(Node.endpoints[self.flavour]) || [], function(name, nextendpoint){
      ...
      var config = Node.endpoints[self.flavour][name];

      self.emit('getsparkendpoint', worker, self.flavour + ':' + name, config, function(error, endpoint){
