...
Allocation.prototype.build = function(finished){
  ...
  */
  async.forEachSeries(Node.stackflavours, function(flavour, nextflavour){
    ...
    */
    var section = Section(flavour, {
      id:self.id + ':' + flavour,
      stackid:self.id,
      flavour:flavour,
      stackconfig:self.stack,
    ...
    section.on('getsparkendpoint', function(worker, name, config, callback){
      callback(null, self.infrastructure.workerendpoint(worker, name, config));
    })

    section.on('deployworker', function(workerconfig, callback){
      self.infrastructure.copystackcode(self.stack, workerconfig, function(error){
        self.infrastructure.deploy(self.stack, workerconfig, callback);  
      })
