...
Section.prototype.spark_node = function(node, workers, callback){
  ...
  */
  async.forEach(workers.containers(), function(worker, nextworker){
    ...
    async.forEach(Node.endpoints[self.flavour] || [], function(name, nextendpoint){
      self.emit('sparkendpoint', worker, self.flavour + ':' + name, function(error, endpoint){
      ...
      worker.append(spark).ship(nextworker);
