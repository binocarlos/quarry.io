...
var log = require('logule').init(module, 'Node Group');
...
Section.prototype.spark = function(callback){
  ...
  */
  ...
  this.defaultnode = Container.new('node', {
    name:this.id + ' ' + this.flavour + ' default node',
    stackid:this.stackid,
    flavour:this.flavour
  }).addClass(this.flavour)

  _.each(self.servicedb.attr('services'), function(service){
    eyes.inspect(self.servicedb.toJSON());
      console.log('-------------------------------------------');
      //process.exit();
  })
