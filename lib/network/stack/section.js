...
Section.prototype.build_workers = function(callback){
  var self = this;
  async.forEach(this.instances.containers(), function(instance, nextinstance){
    self.build_worker(instance, function(error, worker){
...
*/
Section.prototype.build_worker = function(instance, callback){
    ...
    */
    ...
    worker
      ...
      .listen('heartbeat', function(counter){
        log.info('heartbeat: ' + worker.quarryid() + ' : ' + counter);
      })
...
Section.prototype.spark = function(callback){
  ...
  this.build_default_node(function(error, node){
    ...
    */
    ...
    var worker = Deck.pick(self.workers);

    if(!worker){
      throw new Error('No worker found for node: ' + node.summary());
    }

    self.spark_node(node, worker, callback);
  })
}
/*
...
Section.prototype.build_default_node = function(callback){
  ...
  */
  ...
  this.defaultnode = Container.new('node', nodeconfig).addClass(this.flavour);
  ...
  _.each(self.servicedb.attr('services') || [], function(serviceconfig){
    ...
    this.defaultnode.append(service);
  })
...
*/
Section.prototype.spark_node = function(node, worker, callback){

  var self = this;

  log.info('sparking node: ' + node.summary());

  /*
  
    get a softlink to the node
    
  */
  var link = node.hardlink();

  worker.append(link).ship(function(){
    console.log('-------------------------------------------');
    console.log('node link added');
    eyes.inspect(link.toJSON());
  })
