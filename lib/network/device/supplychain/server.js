...
function SupplyChainServer(options){
  ...
  function handle(packet, callback){
    ...
    var frames = packet._frames;

    var header = JSON.parse(frames[frames.length-1]);
    ...
    req.setHeader('x-json-receptionheader', header);
    ...
    function bootstrap_request(bootstrapreq){
      ...
      */
      ...
      /*
        ...
        a branch has happened - req is the new request to send to the holdingbay
        
      */
      bootstrapreq.on('branch', function(req, res){
        var header = bootstrapreq.getHeader('x-json-receptionheader');

        if(!header || !header.bayid){
          return;
        }
        ...
        var responseheader = res.getHeader('x-json-receptionheader') || {};
        responseheader.branches || (responseheader.branches = []);
        responseheader.branches.push(branch.id);
        res.setHeader('x-json-receptionheader', responseheader);

        self.switchboard.broadcast('holdingbay.' + header.bayid, branch);
        */
      })
    }
