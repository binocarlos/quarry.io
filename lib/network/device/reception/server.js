...
ReceptionServer.prototype.bind = function(endpoints){
  ...
  log.info(dye.red('binding') + ' FRONT: ' + dye.red(endpoints.front) + ' - ' + dye.yellow(this.front.identity));
  this.front.bindSync(endpoints.front);
  log.info(dye.red('binding') + ' BACK: ' + dye.red(endpoints.back) + ' - ' + dye.yellow(this.back.identity));
  this.back.bindSync(endpoints.back);
  log.info(dye.red('binding') + ' HTTP: ' + dye.red(endpoints.http.port));
