...
Bootloader.prototype.pubsub = function(callback){
  ...
  var pubsub = node.load();

  callback && callback(null, pubsub);
}
...
Bootloader.prototype.rpc = function(callback){
  ...
  warehouse.prepare(function(ready_callback){
    ...
    warehouse.boot();
    warehouse.assign_network_client(network);
    warehouse.inject_deployment_to_requests(network);
    warehouse.automount(middleware);
