...
//
// also uses the event_callback to emit packets that have been processed
api.supply_chain_factory = function(actions, event_callback){
  ...
  return function(packet, callback){
    ...
    if(actions[packet.action]){
      ...
      function filter_callback(error, packet){

        // route answer via event function
        if(!error){
          event_callback && event_callback(error, packet);
        }
      ...
      actions[packet.action].apply(this, [packet, filter_callback]);
    }
    else{
      packet.answer = {
        ok:false,
...
api.reduce_select = function(endpoint, router){
